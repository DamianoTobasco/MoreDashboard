...
import { Users, ExternalLink, ChevronRight, ArrowUpDown, Search } from 'lucide-react';
...
const TokenHolders: React.FC<TokenHoldersProps> = ({ isLoading }) => {
  ...
  useEffect(() => {
    ...
    const filtered = holders.filter(holder => 
      ...
      (holder.tag && holder.tag.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  ...
  const getAnimationClass = (index: number) => {
    ...
    return `opacity-100 transition-all duration-300 animate-fadeIn delay-${Math.min(index * 100, 500)}`;
  };
  ...
  return (
    <div className="p-1">
              ...
              <div key={holder.address} className={getAnimationClass(index)}>
                <div className="grid grid-cols-12 gap-2 p-3 text-sm hover:bg-black/30 transition-colors">
                  ...
                  <div className="col-span-1 flex items-center justify-end">
                    <button
                      onClick={() => setExpandedHolder(expandedHolder === holder.address ? null : holder.address)}
                ...
                {expandedHolder === holder.address && (
                  <div className="bg-black/20 p-4 text-sm animate-fadeIn">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="text-gray-400 mb-2">Transaction History</h4>
                    ...
                    <div className="mt-4">
                      <h4 className="text-gray-400 mb-2">Recent Transactions</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center py-1 text-xs border-b border-gray-800/50">
                          <span className="text-custom-green">Received 12,450 MORE</span>
                        ...
                        <div className="flex justify-between items-center py-1 text-xs border-b border-gray-800/50">
                          <span className="text-custom-green">Received 25,000 MORE</span>
      ...
      <div className="mt-4 flex justify-between items-center text-xs text-gray-400">
        <span>Showing {visibleHolders.length} of {holders.length} holders</span>
        <a
